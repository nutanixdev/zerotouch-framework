...
class CreateNcmAccount(Script):
    ...
    def execute(self, **kwargs):
        try:
            ncm_credential = self.account.get('pc_credential') or self.data['pc_credential']
            try:
                self.account["pc_username"], self.account["pc_password"] = (
                    read_creds(data=self.data, credential=ncm_credential))
            ...
            class AccountTemplate(Account):
                ...
                resources = AccountResources.Ntnx(
                    username=self.account.get("pc_username"),
                    password=self.account.get("pc_password"),
