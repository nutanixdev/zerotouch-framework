...
class DeleteNameServersPe(ClusterScript):
    """
    Class that deletes nameservers in PE
    """
    def __init__(self, data: Dict, **kwargs):
        super(DeleteNameServersPe, self).__init__(data, **kwargs)
    ...
    def execute_single_cluster(self, cluster_ip: str, cluster_details: Dict):
        ...
        try:
            if cluster_details.get("name_servers_list"):
                ...
                self.logger.info(f"Deleting name servers in {cluster_info!r}")
                response = pe_cluster.delete_name_servers(cluster_details["name_servers_list"])
                if response.get("value"):
                    self.logger.info(f"Deleting name servers {cluster_details['name_servers_list']} successful"
                                     f" in {cluster_info!r}!")
                else:
                    raise Exception(f"Could not delete name servers {cluster_details['name_servers_list']}"
                                    f" in {cluster_info!r}")
    ...
    def verify_single_cluster(self, cluster_ip: str, cluster_details: Dict):
        try:
            ...
            # Initial status
            self.results["clusters"][cluster_ip] = {"DeleteNameServers": {}}
            ...
            for name_server in cluster_details["name_servers_list"]:
                # Initial status
                self.results["clusters"][cluster_ip]["DeleteNameServers"][name_server] = "CAN'T VERIFY"
                ...
                if name_server not in current_name_servers_list:
                    self.results["clusters"][cluster_ip]["DeleteNameServers"][name_server] = "PASS"
                else:
                    self.results["clusters"][cluster_ip]["DeleteNameServers"][name_server] = "FAIL"
        except Exception as e:
            self.logger.debug(e)
