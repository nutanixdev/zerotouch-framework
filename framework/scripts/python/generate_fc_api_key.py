...
from .helpers.fc.fc_api_key import FcApiKey
    ...
    def execute(self, **kwargs):
        try:
            if self.alias:
                ...
                if not status:
                    self.exceptions.append(f"Failed to generate FC API Key. Foundation Central service is not enabled '{self.data['pc_ip']}'")
                    return
                else:
