...
from .helpers.state_monitor.vm_ip_monitor import VmIpMonitor
from .helpers.v3.vm import VM, VmPowerState
...
class PowerOnVmPc(Script):
    ...
    def verify(self, **kwargs):
        ...
        if self.vms_to_power_on:
            ...
            try:
                vm_op = VM(self.pc_session)
                # Get Exisiting VM info list from PC
                filter_criteria = f"cluster_name=={self.pe_cluster}"
                ...
                exisitng_vm_list = vm_op.list(timeout=120, filter=filter_criteria, custom_filters={"power_state": VmPowerState.ON})
                self.logger.debug(f"exisitng_vm_list: {len(exisitng_vm_list)}")

                # Check if the VMs created are in existing power ON vm list
                vm_powered_on_list = [vm for vm in exisitng_vm_list if vm.get("spec", {}).get("name") in self.vm_name_list]
                ...
                if len(self.vm_name_list) == len(vm_powered_on_list):
                    ...
                    vms_failed_power_on_list = [val["spec"]["name"] for val in self.vm_list if val not in exisitng_vm_list]
                    self.results["PowerOnVM"].update({vm: "PASS" for vm in vm_powered_on_list})
