    ...
    def execute(self, **kwargs):
        try:
            ...
            for cluster_ip, cluster_details in self.pe_clusters.items():
                for subnet_info in cluster_details.get("networks", []):
                    ...
                    if not cluster_uuid:
                        ...
                        cluster_uuid = pc_cluster.name_uuid_map.get(cluster_name) or pc_cluster.ip_uuid_map.get(cluster_ip)
                        if not cluster_uuid:
                            self.exceptions.append(f"{cluster_name} is not part of the PC: {self.data['pc_ip']!r}. "
                                                   f"Failed to create subnets in {cluster_name}")
                            continue
            ...
            if not subnets_to_create:
                self.logger.warning(f"No subnets to create in {self.data['pc_ip']!r}")
                return

            self.logger.info(f"Trigger batch create API for subnets in {self.data['pc_ip']!r}")
