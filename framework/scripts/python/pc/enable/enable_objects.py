    ...
    def execute(self, **kwargs):
        try:
            ...
            if status in [Service.ENABLED, Service.ENABLING]:
                self.logger.warning(f"SKIP: Objects/ OSS service is already enabled in {self.data['pc_ip']!r}")
                return

            self.logger.info(f"Enabling Objects/ OSS service in {self.data['pc_ip']!r}")
            ...
            if self.task_uuid:
                ...
                if not status:
                    ...
                    self.logger.info(f"Enabled Objects/ OSS service in the PC {self.data['pc_ip']!r}")
