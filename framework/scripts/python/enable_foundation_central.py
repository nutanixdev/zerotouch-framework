...
from .helpers.state_monitor.fc_enabled_monitor import FcEnabledMonitor
...
class EnableFC(Script):
    ...
    def execute(self, **kwargs):
        try:
            ...
            if status:
                self.logger.warning(f"SKIP: Foundation Central service is already enabled '{self.data['pc_ip']}'")
                return

            self.logger.info(f"Enabling Foundation Central service '{self.data['pc_ip']}'")
            ...
            if self.status:
                ...
                if not status:
                    ...
                    self.logger.info(f"Enabled Foundation Central service in the PC '{self.data['pc_ip']}'")
