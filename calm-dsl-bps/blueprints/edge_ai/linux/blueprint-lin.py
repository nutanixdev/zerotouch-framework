...
for bp in bp_input:
    if bp["name"] == "edge-ai-test":
        ...
        for entity in res_subnets.get("entities", []):
            if entity['status']['cluster_reference']['name'] == cluster_name and entity['status'][
                'name'] == subnet_name:
...
class VM_Provision(Service):
    @action
    def NGTTools_Tasks():
        CalmTask.Exec.ssh(name="install_NGT", filename=INSTALL_SCRIPTS_DIRECTORY + "/ngt/install_ngt.sh",
                          target=ref(VM_Provision), )

    @action
    def Configure_VM():
        CalmTask.Exec.ssh(name="ssh_key_copy", filename=INSTALL_SCRIPTS_DIRECTORY + "/ssh_key_copy.sh",
                          target=ref(VM_Provision), )
        CalmTask.Exec.ssh(name="setup", filename=INSTALL_SCRIPTS_DIRECTORY + "/setup.sh", target=ref(VM_Provision), )
        CalmTask.Exec.ssh(name="validate driver", filename=INSTALL_SCRIPTS_DIRECTORY + "/validate_driver.sh",
                          target=ref(VM_Provision), )
...
class AHVVM_Small(Substrate):
    ...
    readiness_probe = readiness_probe(connection_type="SSH", disabled=False, retries="5", connection_port=22,
                                      address="@@{platform.status.resources.nic_list[0].ip_endpoint_list[0].ip}@@",
                                      delay_secs="30", credential=ref(BP_CRED_cred_os), )
    # update CPU, Memory based on environment specific configs =============================================vvvvvv
    provider_spec.spec["resources"]["num_sockets"] = TSHIRT_SPECS["linux-os"]["global"]["tshirt_sizes"]["small"][
        "num_sockets"]
    provider_spec.spec["resources"]["num_vcpus_per_socket"] = \
    TSHIRT_SPECS["linux-os"]["global"]["tshirt_sizes"]["small"]["num_vcpus_per_socket"]
    provider_spec.spec["resources"]["memory_size_mib"] = TSHIRT_SPECS["linux-os"]["global"]["tshirt_sizes"]["small"][
        "memory_size_mib"]
    # update nic ===========================================================================================vvvvvv
    ...
    # update image ==========================================================================================vvvvvv
    provider_spec.spec["resources"]["disk_list"][0]["data_source_reference"]["name"] = str(
        image_name_uuid_list[0]['name'])
    provider_spec.spec["resources"]["disk_list"][0]["data_source_reference"]["uuid"] = str(
        image_name_uuid_list[0]['uuid'])
...
class AHVVM_Medium(AHVVM_Small):
    ...
    readiness_probe = readiness_probe(connection_type="SSH", disabled=False, retries="5", connection_port=22,
                                      ...
                                      delay_secs="30", credential=ref(BP_CRED_cred_os), )
    # update CPU, Memory based on environment specific configs =============================================vvvvvv
    provider_spec.spec["resources"]["num_sockets"] = TSHIRT_SPECS["linux-os"]["global"]["tshirt_sizes"]["medium"][
        "num_sockets"]
    provider_spec.spec["resources"]["num_vcpus_per_socket"] = \
    TSHIRT_SPECS["linux-os"]["global"]["tshirt_sizes"]["medium"]["num_vcpus_per_socket"]
    provider_spec.spec["resources"]["memory_size_mib"] = TSHIRT_SPECS["linux-os"]["global"]["tshirt_sizes"]["medium"][
        "memory_size_mib"]
    # update nic ===========================================================================================vvvvvv
    provider_spec.spec["resources"]["nic_list"][0]["subnet_reference"]["name"] = str(net_name_uuid_list[0]['name'])
    provider_spec.spec["resources"]["nic_list"][0]["subnet_reference"]["uuid"] = str(net_name_uuid_list[0]['uuid'])
...
class AHVVM_Large(AHVVM_Small):
    ...
    readiness_probe = readiness_probe(connection_type="SSH", disabled=False, retries="5", connection_port=22,
                                      ...
                                      delay_secs="30", credential=ref(BP_CRED_cred_os), )
    # update CPU, Memory based on environment specific configs =============================================vvvvvv
    provider_spec.spec["resources"]["num_sockets"] = TSHIRT_SPECS["linux-os"]["global"]["tshirt_sizes"]["large"][
        "num_sockets"]
    provider_spec.spec["resources"]["num_vcpus_per_socket"] = \
    TSHIRT_SPECS["linux-os"]["global"]["tshirt_sizes"]["large"]["num_vcpus_per_socket"]
    provider_spec.spec["resources"]["memory_size_mib"] = TSHIRT_SPECS["linux-os"]["global"]["tshirt_sizes"]["large"][
        "memory_size_mib"]
    # update nic ===========================================================================================vvvvvv
    provider_spec.spec["resources"]["nic_list"][0]["subnet_reference"]["name"] = str(net_name_uuid_list[0]['name'])
    provider_spec.spec["resources"]["nic_list"][0]["subnet_reference"]["uuid"] = str(net_name_uuid_list[0]['uuid'])
...
class Common(Profile):
    os_cred_public_key = CalmVariable.Simple.Secret(BP_CRED_cred_os_public_KEY, label="OS Cred Public Key",
                                                    is_hidden=True, description="SSH public key for OS CRED user.")
    NFS_PATH = CalmVariable.Simple("", label="NFS Share Path",
                                   regex="^(?:[0-9]{1,3}\.){3}[0-9]{1,3}:(\/[a-zA-Z0-9_-]+)+$", validate_regex=True,
                                   is_mandatory=True, is_hidden=False, runtime=True,
                                   description="Enter the path to your IP NFS share.  For example 10.10.10.10:/sharename")
    NFS_MOUNT_POINT = CalmVariable.Simple("/mnt/data", label="NFS Mount Point", is_mandatory=False, is_hidden=True,
                                          runtime=False, description="Local NFS Mount Point")
    WORKER = CalmVariable.Simple("1", label="", is_mandatory=False, is_hidden=False, runtime=True, description="")
    NVIDIA_DRIVER_VERSION = CalmVariable.WithOptions.Predefined.string(["515.86.01"],
                                                                       label="Please select the NVidia driver version to be used.",
                                                                       default="515.86.01", is_mandatory=True,
                                                                       is_hidden=False, runtime=True, description="", )
    NFS_WORKING_DIRECTORY = CalmVariable.WithOptions(["nai-dl-bench-"], label="AI Training Working Directory",
                                                     default="nai-dl-bench-", is_mandatory=True, is_hidden=False,
                                                     runtime=True, description="", )

    @action
    def NaiDlBench_Data_Setup(name="NAI DL Bench Data Setup"):
        CalmTask.Exec.ssh(name="NaiDlBench_Data_Setup", filename=DAY2_SCRIPTS_DIRECTORY + "/nai_dl_bench_data.sh",
                          target=ref(VM_Provision), )
        NAI_DL_BENCH_VERSION = CalmVariable.WithOptions.Predefined.string(["0.2.3"],
                                                                          label="Please select the version to be used.",
                                                                          default="0.2.3", is_mandatory=True,
                                                                          is_hidden=False, runtime=True,
                                                                          description="", )

    @action
    def AITraining(name="AI Training"):
        CalmTask.Exec.ssh(name="AI Training", filename=DAY2_SCRIPTS_DIRECTORY + "/ai_training.sh",
                          target=ref(VM_Provision), )
        NAI_DL_BENCH_VERSION = CalmVariable.WithOptions.Predefined.string(["0.2.3"],
                                                                          label="Please select the version to be used.",
                                                                          default="0.2.3", is_mandatory=True,
                                                                          is_hidden=False, runtime=True,
                                                                          description="", )
        AI_TRAINING_OUTPUT_FOLDER = CalmVariable.Simple("training-output", label="AI Training Output Folder",
                                                        is_mandatory=True, is_hidden=False, runtime=True,
                                                        description="", )
        AI_TRAINING_OUTPUT_FILE = CalmVariable.Simple("resnet.pth", label="AI Training Output File", is_mandatory=True,
                                                      is_hidden=False, runtime=True, description="", )
        EXTRA_PARAMS = CalmVariable.Simple("", label="AI Training Optional Parameters", is_mandatory=False,
                                           is_hidden=False, runtime=True,
                                           description="OPTIONAL - Leave blank if not needed - Enter any extra parameters needed, e.g., --quiet, etc.", )

    @action
    def AIBatchInference(name="AI Batch Inference"):
        CalmTask.Exec.ssh(name="AI Batch Inference", filename=DAY2_SCRIPTS_DIRECTORY + "/ai_inference.sh",
                          target=ref(VM_Provision), )
        NAI_DL_BENCH_VERSION = CalmVariable.WithOptions.Predefined.string(["0.2.3"],
                                                                          label="Please select the version to be used.",
                                                                          default="0.2.3", is_mandatory=True,
                                                                          is_hidden=False, runtime=True,
                                                                          description="", )
        AI_TRAINING_OUTPUT_FOLDER = CalmVariable.Simple("", label="AI Training Output Folder", is_mandatory=False,
                                                        is_hidden=False, runtime=True,
                                                        description="OPTIONAL - Leave blank if not needed", )
        AI_TRAINING_OUTPUT_FOLDER_DEFAULT = CalmVariable.Simple("training-output",
                                                                label="AI Training Output Folder Default Value",
                                                                is_mandatory=False, is_hidden=True, runtime=False,
                                                                description="Default value for the AI Training Output Folder", )
        AI_TRAINING_OUTPUT_FILE = CalmVariable.Simple("", label="AI Training Output File", is_mandatory=False,
                                                      is_hidden=False, runtime=True,
                                                      description="OPTIONAL - Leave blank if not needed", )
        AI_TRAINING_OUTPUT_FILE_DEFAULT = CalmVariable.Simple("resnet.pth",
                                                              label="AI Training Output File Default Value",
                                                              is_mandatory=False, is_hidden=True, runtime=False,
                                                              description="Default value for the AI Training Output File", )
        EXTRA_PARAMS = CalmVariable.Simple("", label="AI Inference Optional Extra Parameters", is_mandatory=False,
                                           is_hidden=False, runtime=True,
                                           description="OPTIONAL - Leave blank if not needed - Enter any extra parameters needed, e.g., --quiet, etc.", )
        AI_MODEL_NAME = CalmVariable.Simple("", label="AI Training Model Name", is_mandatory=False, is_hidden=False,
                                            runtime=True,
                                            description="OPTIONAL - Leave blank if not needed - Enter the AI model name if not using the default value", )
        AI_MODEL_NAME_DEFAULT = CalmVariable.Simple("resnet50", label="AI Training Model Name Default Value",
                                                    is_mandatory=False, is_hidden=True, runtime=False,
                                                    description="Default value for the AI model name", )

    @action
    def AIStartInferenceService(name="AI Start Inference Service"):
        CalmTask.Exec.ssh(name="AI Start Inference Service", filename=DAY2_SCRIPTS_DIRECTORY + "/ai_inference_start.sh",
                          target=ref(VM_Provision), )
        NAI_DL_BENCH_VERSION = CalmVariable.WithOptions.Predefined.string(["0.2.3"],
                                                                          label="Please select the version to be used.",
                                                                          default="0.2.3", is_mandatory=True,
    ...
    def AIStopInferenceService(name="AI Stop Inference Service"):
        CalmTask.Exec.ssh(name="AI Stop Inference Service", filename=DAY2_SCRIPTS_DIRECTORY + "/ai_inference_stop.sh",
                          target=ref(VM_Provision), )
...
class AHV_Small(Common):
    ...
    def Scaleout(name="Scale Out"):
        increase_count = CalmVariable.Simple("1", label="", is_mandatory=False, is_hidden=False, runtime=True,
                                             description="", )
        CalmTask.Scaling.scale_out("@@{increase_count}@@", name="ScaleOut", target=ref(AHV_Deployment_Sml), )

    @action
    def Scalein(name="Scale In"):
        decrease_count = CalmVariable.Simple("1", label="", is_mandatory=False, is_hidden=False, runtime=True,
                                             description="", )
